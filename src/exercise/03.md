...
### You probably don't need `useContext`

Most people use `useContext` to avoid prop drilling. But actually it's not
necessary. Most of the time, you only need `useContext` when you're building a
library. The React documentation says that to avoid prop drilling, component
composition is often a simpler solution than context.

Also if you're using context, the component that uses the context can only be
called inside the context provider. If not, it might give us an error, even
though we can mitigate this by passing default value into `createContext`, but
it's just not the right way. Instead, we can change our component composition by
using the `children` props. We put the component that needs the data as the
child of the top component and make the top component renders the children
components. More details on the Michael Jackson video below.
