...
Actually we can set the `current` property of `ref` Without using
`useImperativeHandle`:
...
The problem is this is manipulating a value during the render method and it's
not idempotent, meaning if we run this multiple times it could have some
unexpected results. In fact, we can do it this way:
...
Actually this is a bit closer to what `useImperativeHandle` is doing is it's
setting the `current` property after React has rendered all of our components
and updated the DOM.
