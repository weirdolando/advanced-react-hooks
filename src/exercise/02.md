...
caching strategy React has adopted has a size of 1. That is, they only keep
...
React context. So for React's memoization it's more like this:
...
To be clear, in React's case it's not a `!==` comparing the `prevInput`. It
checks equality of each prop and each dependency individually. Let's check each
one:

- `React.memo`'s `prevInput` is props and `prevResult` is react elements (JSX)
- `React.useMemo`'s `prevInput` is the dependency array and `prevResult` is
  whatever your function returns
- `React.useCallback`'s `prevInput` is the dependency array and `prevResult` is
  the function

### Creating a safe dispatch

Before React 18, let's say you have a logic that fetch data from a server, and
then run `dispatch` function after getting the data. But while the browser is
still fetching the data from the server, the user decided to change the page or
unmount the component. After the fetch is finished and your logic calls the
`dispatch` function, you will get a warning from react (See the warning below at
extra exercise 3). To solve this, we can create our own safe dispatch hook like:
...
Here, we pass the dispatch function from `useReducer` to our custom
`useSafeDispatch` hook. Now, only when the component is mounted, we can do
`dispatch`.
